...
parser.add_argument(
    "--epoch", "-e", default=100, type=int, help="number of epochs to learn"
)
parser.add_argument(
    "--dimz", "-z", default=20, type=int, help="dimention of encoded vector"
)
parser.add_argument(
    "--batchsize", "-b", type=int, default=100, help="learning minibatch size"
)
parser.add_argument(
...
for epoch in six.moves.range(1, n_epoch + 1):
    ...
    print(
        "train mean loss={}, mean reconstruction loss={}".format(
            sum_loss / N, sum_rec_loss / N
        )
    )
    ...
    with chainer.no_backprop_mode():
        for i in six.moves.range(0, N_test, batchsize):
            ...
            writer.add_image(
                "reconstructed", model(x).reshape(-1, 1, 28, 28), epoch * N_test + i
            )
            writer.add_image("input", x.reshape(-1, 1, 28, 28), epoch * N_test + i)
            del model.loss
    print(
        "test  mean loss={}, mean reconstruction loss={}".format(
            sum_loss / N_test, sum_rec_loss / N_test
        )
    )
