...
from .proto.step_stats_pb2 import (
    RunMetadata,
    StepStats,
    DeviceStepStats,
    NodeExecStats,
...
class GraphPy(object):
    ...
    def to_proto(self):
        ...
        for v in self.nodes_io.values():
            ...
            if (
                v.tensor_size and len(v.tensor_size) > 0
            ):  # assume data is float32, only parameter is counted
                node_stats.append(
                    NodeExecStats(
                        node_name=v.debugName,
                        all_start_micros=int(time.time() * 1e7),
...
def graph(model, args, verbose=False, use_cuda=False, **kwargs):
    ...
    with torch.onnx.set_training(model, False):  # TODO: move outside of torch.onnx
        ...
        try:
            ...
            with torch.autograd.profiler.profile(
                record_shapes=True, use_cuda=use_cuda
            ) as prof:
                result = model(*args)
    ...
    stepstats = RunMetadata(
        step_stats=StepStats(
            dev_stats=[DeviceStepStats(device=device, node_stats=node_stats)]
        )
    )
