...
def scalar(name, scalar, display_name="", summary_description=""):
    ...
    metadata = SummaryMetadata(
        display_name=display_name, summary_description=summary_description
    )
    return Summary(
        value=[Summary.Value(tag=name, simple_value=scalar, metadata=metadata)]
    )
...
def audio(tag, tensor, sample_rate=44100):
    ...
    ), "The second dimension should be 1 or 2."
...
def _get_tensor_summary(tag, tensor, content_type, json_config):
    ...
    smd = SummaryMetadata(
        plugin_data=SummaryMetadata.PluginData(plugin_name="mesh", content=content)
    )
